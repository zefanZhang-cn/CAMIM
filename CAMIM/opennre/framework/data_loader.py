...
class SentenceREDataset(data.Dataset):
    ...
    def __init__(self, text_path, pic_path, cap_path, rel2id, tokenizer, sample_ratio, kwargs):
        """
        Args:
            text_path: path of the input text
            pic_path: path of the input image
            cap_path: path of the input caption
        ...
        self.cap_data = []
        f_cap = open(cap_path, encoding='UTF-8')
        f_lines_cap = f_cap.readlines()
        for i2 in tqdm(range(len(f_lines_cap))):
            line = f_lines_cap[i2].rstrip()  # caption的一条数据，由img_id和text组成
            if len(line) > 0:
                dic2 = eval(line)
                self.cap_data.append(dic2)
        f_cap.close()

        logging.info(
            "Loaded caption dataset {} with {} lines and {} relations.".format(cap_path, len(self.cap_data),
                                                                                   len(self.rel2id)))
    ...
    def __getitem__(self, index):
        ...
        # get caption id and text
        for i,line in enumerate(self.cap_data):
        ...
        # caption tokennizer
        encode_caption = self.tokenizer_cap.encode_plus(text=cap_text,max_length=40,truncation=True,padding='max_length')
        caption_input_ids, caption_token_type_ids, caption_attention_mask = encode_caption['input_ids'], encode_caption['token_type_ids'], encode_caption['attention_mask']
        caption_input_ids, caption_token_type_ids, caption_attention_mask = torch.tensor(caption_input_ids).unsqueeze(0), torch.tensor(caption_token_type_ids).unsqueeze(0), torch.tensor(caption_attention_mask).unsqueeze(0)
        ...
        np_pic_dif = np.array(pic_dif).astype(np.float32)
        np_pic_ori = np.array(pic_ori).astype(np.float32)
        np_pic_dif_objects = np.array(pic_dif_objects).astype(np.float32)
        ...
        list_pic_dif = list(torch.tensor(np_pic_dif).unsqueeze(0))
        list_pic_ori = list(torch.tensor(np_pic_ori).unsqueeze(0))
        list_pic_dif_objects = list(torch.tensor(np_pic_dif_objects).unsqueeze(0))
        list_pic_ori_objects = list(torch.tensor(np_pic_ori_objects).unsqueeze(0))
        res = [self.rel2id[item['relation']]] + [img_id] + seq + list_pic_dif + list_pic_ori + list_pic_dif_objects + list_pic_ori_objects + [torch.tensor(weight).reshape(1,4)]+[caption_input_ids]+[caption_token_type_ids]+[caption_attention_mask]
...
def SentenceRELoader(text_path,  pic_path, cap_path, rel2id, tokenizer,
                     batch_size, shuffle, num_workers=8, sample_ratio=1.0, collate_fn=SentenceREDataset.collate_fn, **kwargs):
    dataset = SentenceREDataset(text_path=text_path, pic_path=pic_path, cap_path=cap_path,
                                rel2id=rel2id,
                                tokenizer=tokenizer,
